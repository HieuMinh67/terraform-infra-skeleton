resource "tfe_workspace" "this" {
  ...
  working_directory = each.value.is_vcs_connected ? (var.state != "destroyed" ? "platforms/${var.platform}/${each.value.app_type}/${each.value.app_category}/${each.value.app_name}" : "platforms/${var.platform}/${each.value.app_type}/tfc/workspace/empty") : null

  dynamic "vcs_repo" {
    for_each = each.value.is_vcs_connected ? [1] : []
    content {
      identifier     = "BeanTraining/terraform-infra-${var.environment}"
